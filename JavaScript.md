...
* Що таке `ArrayBuffer`? У чому різниця між `Uint32Array` і `Float32Array`?
  * `ArrayBuffer` використовується для роботи з бінарними даними
...
* Що таке `descriptor`?
  * Дескриптор `descriptor` – об'єкт, який описує властивості поведінки
    ...
    * `writable` – значення властивості можна змінювати, якщо `true`. За умовчанням `false`.
    * `configurable` – якщо true, це властивість можна видалити, а також змінити його надалі за допомогою нових викликів `defineProperty`. За умовчанням `false`.
    * `enumerable` – якщо істина, це властивість переглядається в циклі `for..in` і методі `Object.keys()`. За умовчанням `false`
    * `get` – функція, яка повертає значення властивості. За умовчанням не визначено
    * `set` – функція, яка записує значення властивості. За умовчанням не визначено
